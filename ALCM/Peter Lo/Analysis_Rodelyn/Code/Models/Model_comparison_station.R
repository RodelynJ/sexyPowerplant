...
#----------------------------------#
#                                  #
#     summarising by station        #
#                                  #
#----------------------------------#

#----------------------------------------------------------------------------------------------------------------------

traps_DF2 <- traps_DF %>% group_by(station, Season,Season2, week_diff) %>%
...
ggplot(traps_DF2[traps_DF2$Season2 %in% 15,], aes(x=week_diff, y=rate_norm)) + geom_line() + facet_wrap(~station) +
  geom_line(data=traps_DF2[traps_DF2$Season2 %in% 15,], aes(x=week_diff, y=mean_rate), col="red")
...
#----------------------------------------------------------------------------------------------------------------------
tree_pred_DF <- traps_DF2[,c("Season","Season2","station","rate_norm","week_diff")]

tree_pred_DF$pred_ratenorm<- NA
j=1

for(j in 1:length(unique(traps_DF$station))){
  df <- traps_DF2[traps_DF2$station %in% unique(traps_DF2$station)[j],]
  ...
  for(i in 1:n_length){
    ...
    tree_pred_DF[tree_pred_DF$station %in% unique(traps_DF2$station)[j] & tree_pred_DF$Season2 %in% unique(df$Season2)[i],c("pred_ratenorm")] <- tree_pred  
...
tree_rate_plots <- ggplot(tree_pred_DF, aes(x=week_diff, y=rate_norm, group=station)) + geom_line() + 
  facet_wrap(~interaction(station, Season)) + 
  geom_line(data=tree_pred_DF, aes(x=week_diff, y=pred_ratenorm, group=station),col="red")
...
#----------------------------------------------------------------------------------------------------------------------
RF_pred_DF <- traps_DF2[,c("Season","Season2","station","rate_norm","week_diff")]
...
for(j in 1:length(unique(traps_DF$station))){
  ...
  for(i in 1:n_length){
    ...
    RF_pred_DF[RF_pred_DF$station %in% unique(traps_DF2$station)[j] & RF_pred_DF$Season2 %in% unique(df$Season2)[i],c("pred_ratenorm")] <- tree_pred  
...
RF_rate_plots <- ggplot(RF_pred_DF, aes(x=week_diff, y=rate_norm, group=station)) + geom_line() + 
  facet_wrap(~interaction(station, Season)) + 
  geom_line(data=RF_pred_DF, aes(x=week_diff, y=pred_ratenorm, group=station),col="red")


table(traps_DF2$station)
#----------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------------------
SVM_pred_DF <- traps_DF2[,c("Season","Season2","station","rate_norm","week_diff")]
...
for(j in 1:length(unique(traps_DF$station))){
  ...
  for(i in 1:n_length){
    ...
    SVM_pred_DF[SVM_pred_DF$station %in% unique(traps_DF2$station)[j] & SVM_pred_DF$Season2 %in% unique(df$Season2)[i],c("pred_ratenorm")] <- tree_pred  
...
SVM_rate_plots <- ggplot(SVM_pred_DF, aes(x=week_diff, y=rate_norm, group=station)) + geom_line() + 
  facet_wrap(~interaction(station, Season)) + 
  geom_line(data=SVM_pred_DF, aes(x=week_diff, y=pred_ratenorm, group=station),col="red")
...
#----------------------------------------------------------------------------------------------------------------------
gbm_pred_DF <- traps_DF2[,c("Season","Season2","station","rate_norm","week_diff")]
...
for(j in 1:length(unique(traps_DF$station))){
  ...
  for(i in 1:n_length){
    ...
    gbm_pred_DF[gbm_pred_DF$station %in% unique(traps_DF2$station)[j] & gbm_pred_DF$Season2 %in% unique(df$Season2)[i],c("pred_ratenorm")] <- tree_pred  
...
GBM_rate_plots <- ggplot(gbm_pred_DF, aes(x=week_diff, y=rate_norm, group=station)) + geom_line() + 
  facet_wrap(~interaction(station, Season)) + 
  geom_line(data=gbm_pred_DF, aes(x=week_diff, y=pred_ratenorm, group=station),col="red")
